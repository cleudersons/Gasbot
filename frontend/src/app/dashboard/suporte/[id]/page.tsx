...
export default function TicketPage() {
  ...
  async function carregar() {
    const r = await fetch(`/api/suporte/tickets/${params.id}`);
    if (!r.ok) {
  ...
  async function enviar(e: React.FormEvent) {
    ...
    setEnviando(true);
    const r = await fetch(`/api/suporte/tickets/${params.id}/mensagens`, {
      method: 'POST',
  ...
  return (
    <div className="max-w-3xl">
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[calc(100vh-200px)]">
              ...
              >
                ...
                <p
                  className={`text-[10px] mt-1 ${
                    m.autor === 'user' ? 'text-orange-100' : 'text-gray-400'
                  }`}
                >
        ...
        {ticket.status === 'fechado' ? (
          <div className="border-t border-gray-200 px-5 py-3 text-sm text-gray-500 text-center">
            Esse ticket está fechado. Abra um novo se precisar de mais ajuda.
          </div>
        ) : (
          <form
            onSubmit={enviar}
